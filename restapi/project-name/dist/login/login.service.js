...
let LoginService = exports.LoginService = LoginService_1 = class LoginService {
    ...
    async logIn(req) {
        try {
            const user = await this.loginProvider.findOneWithPass(req.body.username);
    ...
    async refreshToken(req) {
        try {
            if (jsonwebtoken_1.default.verify((req.headers.authorization).split(' ')[1], process.env.JWT_REFRESH_SECRET)) {
                const user = jsonwebtoken_1.default.verify((req.headers.authorization).split(' ')[1], process.env.JWT_REFRESH_SECRET);
                return {
                    access_token: this.jwtService.sign({
                        name: user['name'],
                        profession: user['profession'],
