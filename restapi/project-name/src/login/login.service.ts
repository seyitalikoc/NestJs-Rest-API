...
export class LoginService {
  ...
  async refreshToken(@Request() req){
    try{
      if( jwt.verify((req.headers.authorization).split(' ')[1], process.env.JWT_REFRESH_SECRET)){
        const user = jwt.verify((req.headers.authorization).split(' ')[1], process.env.JWT_REFRESH_SECRET);
